...
export default function RecentActivityCard({ data }) {
  ...
  const { glow, titleLanguage } = anitingContext
  ...
  return (
    ...
    >
      <div className="absolute top-0 h-72 w-48">
        <div className="absolute bottom-0 left-0 z-10 line-clamp-1 flex h-6 w-48 rounded-b-sm bg-[#00000090] px-1 text-sm tracking-wide drop-shadow-3xl backdrop-blur-sm">
          {getTitle(data?.media?.title, titleLanguage)}
        </div>
      </div>
      <img
