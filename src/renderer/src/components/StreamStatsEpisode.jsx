...
export default function StreamStatsEpisode({
  ...
  const { autoUpdateAnilistEpisode } = useAnitingContext()
  ...
  useEffect(() => {
    ...
    if (
      ...
      try {
        ...
        toast('¡Episodio actualizado en AniList!', { type: 'success' })
      } catch (error) {
        console.error('Error updating episode on AniList:', error)
        toast('¡Error al actualizar episodio en AniList!', { type: 'error' })
      }
    }
    ...
    <div className="mb-10 mt-3 flex flex-col items-center gap-y-1 border-b border-gray-700 pb-3 font-space-mono">
      ...
      <div className="">
        <div className="mt-3 flex justify-center gap-x-20">
          ...
          <p className="opacity-45">
            <strong>Progreso:</strong> {(details?.progress * 100)?.toFixed(2)}%
          </p>
          <Button
            size="1"
            color="red"
