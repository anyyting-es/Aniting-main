...
export default function AnimeCard({ data }) {
  ...
  function handleClick() {
    navigate(`/${data.type === 'MANGA' ? 'manga' : 'anime'}/${data.id}`, {
      state: { data }
    })
  }
  ...
  const { glow, hoverCard, titleLanguage } = anitingContext
    ...
    >
      {
        <div
          onClick={() => handleClick()}
          className={`group relative flex w-[10.6rem] animate-fade cursor-pointer flex-col items-center justify-center gap-y-2 transition-all duration-300 ease-in-out hover:-translate-y-1 hover:scale-[1.03] hover:brightness-110`}
          style={{
            transition:
        ...
        >
          ...
          <div className="z-20 flex w-full flex-col gap-y-1">
            <div
              title={getTitle(data?.title, titleLanguage)}
              className="line-clamp-2 h-11 w-full text-sm font-medium opacity-90"
            >
              {getTitle(data?.title, titleLanguage)}
            </div>
            ...
            <div className="flex justify-between text-xs">
              <div className="flex items-center gap-x-2">
                ...
                {status === 'CURRENT' && (
                  <Tooltip content="Viendo">
                    <p className="h-2 w-2 rounded-full bg-blue-500"></p>
                  </Tooltip>
                )}
                {status === 'PLANNING' && (
                  <Tooltip content="Planeado">
                ...
                {status === 'DROPPED' && (
                  <Tooltip content="Abandonado">
                    <p className="h-2 w-2 rounded-full bg-red-700"></p>
                  </Tooltip>
                )}
                {status === 'PAUSED' && (
                  <Tooltip content="Pausado">
      ...
      {/* {true && ( */}
      {/* Popup card deshabilitado - solo animación suave */}
      {false && (
        <div
          ...
          <div className="flex flex-col gap-y-2">
            ...
            <div className="flex justify-center">
              <div className="mx-2 flex w-full items-center justify-between gap-x-2 border-b border-[#545454] pb-1 text-xs text-gray-300">
                ...
                <p>{`${data?.type === 'MANGA' ? (data?.chapters ? data?.chapters : '?') : data?.episodes ? data?.episodes : '?'} ${data?.type === 'MANGA' ? 'caps' : 'eps'}`}</p>
                <div className="h-5 w-[1px] bg-[#333]"></div> {/* Divider */}
                {data.averageScore && (
                  <>
