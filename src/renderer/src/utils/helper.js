...
export function getTitle(titleObj, preference) {
  if (!titleObj) return ''
  if (preference === 'english' && titleObj.english) return titleObj.english
  if (preference === 'native' && titleObj.native) return titleObj.native
  return titleObj.romaji || titleObj.english || titleObj.native || ''
}

export function isFiller(animeId, episodeNumber, fillersData) {
  if (!animeId || !episodeNumber || !fillersData) return false
  const fillers = fillersData[String(animeId)]
  if (!fillers) return false

  for (const item of fillers) {
    if (typeof item === 'number' && item === episodeNumber) return true
...
export async function searchAnime(text, limit = 10) {
  try {
    ...
    const isAdultStr = contentType === 'HENTAI' ? ', isAdult: true' : ', isAdult: false'

    const query = `
      query ($search: String, $limit: Int) {
        Page(perPage: $limit) {
          media(search: $search, type: ${typeStr}${isAdultStr}) {
            id
            format
            status
            episodes
...
export async function searchAiringAnime(text, limit = 2) {
  ...
  try {
    ...
    const query = `
      query ($search: String, $limit: Int) {
        Page(perPage: $limit) {
          media(search: $search, type: ${typeStr}, status: RELEASING${isAdultStr}) {
            id
            format
            status
            episodes
...
export async function getTopAiringAnime() {
  const showAdult = localStorage.getItem('showAdultContent') === 'true'
  const isAdultStr = showAdult ? 'isAdult: true' : 'isAdult: false'

  const query = `
    query {
      Page(perPage: 49, page: 1) {
        media(type: ANIME, sort: TRENDING_DESC, status_in: [RELEASING], ${isAdultStr}) {
          ${anilistQueryObject}
        }
      }
    }
  ...
  const query = `
    query ($page: Int) {
      Page(page: $page, perPage: 25) {
        media(type: ANIME, sort: SCORE_DESC, ${isAdultStr}) {
          ${anilistQueryObject}
        }
      }
    }
...
export async function getAnimeById(id) {
  return getMediaById(id, 'ANIME')
}

export async function getMangaById(id) {
  return getMediaById(id, 'MANGA')
}

export async function getMediaById(id, mediaType = 'ANIME') {
  console.log(`Fetching ${mediaType.toLowerCase()} with id:`, id)
...
export async function getAniZipMappings(anilist_id, anidb = false) {
  try {
    ...
    if (response.status === 500) {
      throw new Error(
        'Internal server error. The DB is likely being updated, might take a couple of minutes.'
      )
    }
...
export async function searchAnilist(searchObject, page = 1, perPage = 30) {
  ...
  if (searchObject?.watchStatus && statusMap.includes(searchObject.watchStatus)) {
    ...
    query = `
      query {
        MediaListCollection(userId: ${searchObject.userId}, type: ${typeStr}, sort: UPDATED_TIME_DESC, status: ${searchObject.watchStatus}, forceSingleCompletedList: true) {
          lists {
            status,
            entries {
               media {
